...
float volumetric_multiplier[MAX_EXTRUDERS] = { 1.0
#if MAX_EXTRUDERS > 1
, 1.0
#if MAX_EXTRUDERS > 2
, 1.0
...
#endif
float extruder_offset[NUM_EXTRUDER_OFFSETS][MAX_EXTRUDERS] = {
...
#if MAX_EXTRUDERS == 1 || defined(COREXY) \
    ...
    else if (code_seen('M'))
    {
...
#endif //TEMP_BED_PIN
            for (int8_t cur_extruder = 0; cur_extruder < INSTALLED_EXTRUDERS; ++cur_extruder) {
        ...
        case 200: // M200 D<millimeters> set filament diameter and set E axis units to cubic millimeters (use S0 to set back to millimeters).
        {
            ...
            if (code_seen('T')) {
                ...
                if (tmp_extruder >= INSTALLED_EXTRUDERS) {
                    SERIAL_ECHO_START;
        ...
        case 218: // M218 - set hotend offset (in mm), T<extruder_number> X<offset_on_X> Y<offset_on_Y>
            if (INSTALLED_EXTRUDERS > 1)
            {
                ...
                for (tmp_extruder = 0; tmp_extruder < INSTALLED_EXTRUDERS; tmp_extruder++)
                {
                    SERIAL_ECHO(" ");
        ...
        case 301: // M301
        {
            // BUGBUG - now this is lame: M301 can only have 1 set of values and can't handle multiple extruders! so we reset only the first one
            if (code_seen('P')) WORK_Kp(0) = code_value();
            if (code_seen('I')) WORK_Ki(0) = scalePID_i(code_value());
            if (code_seen('D')) WORK_Kd(0) = scalePID_d(code_value());

#ifdef PID_ADD_EXTRUSION_RATE
            if (code_seen('C')) WORK_Kc(0) = code_value();
#endif

            updatePID();
            SERIAL_PROTOCOL(MSG_OK);
            SERIAL_PROTOCOL(" p:");
            SERIAL_PROTOCOL(WORK_Kp(0));
            SERIAL_PROTOCOL(" i:");
            SERIAL_PROTOCOL(unscalePID_i(WORK_Ki(0)));
            SERIAL_PROTOCOL(" d:");
            SERIAL_PROTOCOL(unscalePID_d(WORK_Kd(0)));
#ifdef PID_ADD_EXTRUSION_RATE
            SERIAL_PROTOCOL(" c:");
            //Kc does not have scaling applied above, or in resetting defaults
            SERIAL_PROTOCOL(WORK_Kc(0));
#endif
            SERIAL_PROTOCOLLN("");
        }
            break;
