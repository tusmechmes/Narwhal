...
const char *int4_tostr(const int &x);
const char *long5_tostr(const unsigned long &x);

const char *float3_tostr(const float &x);
const char *float31ns_tostr(const float &x); // float to string without sign character
